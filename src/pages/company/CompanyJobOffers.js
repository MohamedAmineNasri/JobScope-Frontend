    ...
    import {
    getUsersAppliedToJobAction,
    updateUserApplicationStatusAction,
    userLogoutAction,
    userProfileAction,
...
import "@react-pdf-viewer/default-layout/lib/styles/index.css";
    ...
    const CompanyJobOffers = () => {
      ...
      // console.log(
      //   "id:",
      //   user?.createdJobs?.map((job) => ({ _id: job._id }))
      // ); // Add this line
      ...
      //     setJobIds(ids);
      //     dispatch(getUsersAppliedToJobAction(ids));
      //   }
      // }, [dispatch, user]);
            ...
            <div>
              {params.row.editable && (
                <Select
                  ...
                  onChange={(event) =>
                    handleAvailabilityChange(params.row._id, event.target.value)
                  }
      ...
      const handleAvailabilityChange = (jobId, newAvailability) => {
        dispatch(updateJobAvailability(jobId, newAvailability === "Available"));
      };
      // In your component
